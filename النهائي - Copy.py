...
class Booking(db.Model):
    ...
    def __repr__(self):
        return (f"Booking('{self.name}', '{self.car_number}', '{self.car_type}', '{self.phone_number}',"
                f" '{self.date}', '{self.time}', '{self.datetime}', '{self.space_id}')")
...
def AMA():
    ...
    if request.method == 'POST':
        ...
        def is_space_booked(space_id, date, time):
            return Booking.query.filter_by(space_id=space_id, date=date, time=time).first() is not None
        ...
        if booking_status:
           ...
           # return "This parking space is already booked."
            ...
            new_booking = Booking(name=name, car_number=car_number, car_type=car_type, phone_number=phone_number,
                                  date=date, time=time, space_id=space_id)
            ...
            booking_details = (f"Name: {name}, Car Number: {car_number}, Car Type: {car_type}, Phone Number: {phone_number},"
                               f" Date: {date}, Time: {time}, Space ID: {space_id}")
...
def check_database_for_data(data):
    with app.app_context():
        try:
            ...
            booking = Booking.query.filter_by(name=data.split(',')[0].split(': ')[1]).first()
            ...
            if booking:
                ...
                print(mqtt_topic1,"Message content:", message_content)  # طباعة نص الرسالة المُرسلة
                #return "Booking found: " + str(booking)
            else:
                print("No booking found with this data")
                #return "No booking found with this data"
        except ValueError:
